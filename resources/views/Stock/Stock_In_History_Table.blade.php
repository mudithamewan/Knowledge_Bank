...
<style>
    ...
    .dropdown-filter {
        display: none;
        position: fixed;
        /* important: avoid clipping by scroll container */
        background: #fff;
...
<div class="card">
    <div class="card-header bg-transparent border-bottom">
        <div class="d-flex flex-wrap align-items-start">
            <div class="me-2">
                <h5 class="card-title mt-2 text-primary">HOSTORY DETAILS</h5>
            </div>
            <div class="ms-auto">
                <form action="{{url('/')}}/get_stock_in_filter_result_table" method="post">
                    @csrf
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table id="result_table" class="table table-sm">
                <thead>
                    <tr>
                        <th style="white-space: nowrap;">IN DATE <span class="filter-icon"><i class="bx bx-filter"></i></span></th>
                        <th style="white-space: nowrap;">IN BY <span class="filter-icon"><i class="bx bx-filter"></i></span></th>
                        <th style="white-space: nowrap;">GRN <span class="filter-icon"><i class="bx bx-filter"></i></span></th>
                        <th style="white-space: nowrap;">TOTAL ITEMS <span class="filter-icon"><i class="bx bx-filter"></i></span></th>
                        <th style="white-space: nowrap;">PURCHASE PRICE <span class="filter-icon"><i class="bx bx-filter"></i></span></th>
                        <th style="white-space: nowrap;">SELLING PRICE <span class="filter-icon"><i class="bx bx-filter"></i></span></th>
                        ...
                        <th style="white-space: nowrap;">REMARK <span class="filter-icon"><i class="bx bx-filter"></i></span></th>
...
<div class="modal fade" id="in_details_modal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="staticBackdropLabel">DETAIL VIEW</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <span id="LOAD_STOCK_IN_DETAILS_VIEW" class="VIEW_EMPTY"></span>
            </div>
    ...
    function get_table_data() {
        ...
        var table = $('#result_table').DataTable({
            ...
            ajax: {
                ...
                url: "{{url('/')}}/get_stock_in_filter_result_table",
                data: {
                    FROM_DATE: '{{$FROM_DATE}}',
                    TO_DATE: '{{$TO_DATE}}',
                    MW_ID: '{{$MW_ID}}',
            ...
            columns: [{
                ...
                {
                    data: 'sil_total_items',
                    render: renderNA
                },
                {
                    data: 'sil_purchase_amount',
                ...
                {
                    data: 'sil_id',
                    render: d => `<button class="btn btn-outline-primary btn-sm"
                        data-bs-toggle="modal" data-bs-target="#in_details_modal"
                        onclick="ajax_action(
                            '{{ url('/') }}/load_stock_in_detail_view',
                            'LOAD_STOCK_IN_DETAILS',
                            JSON.stringify({ SIL_ID: ${d} }),
                            '{{ csrf_token() }}')">VIEW</button>`
                }
            ],
            initComplete: function() {
                ...
                $('#result_table thead th').each(function(i) {
                    ...
                    // attach dropdown to body so it's not affected by table's scroll container
                    ...
                    // Toggle dropdown and position it under the filter icon

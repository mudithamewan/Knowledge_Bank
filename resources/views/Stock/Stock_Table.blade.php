...
<div class="card">
    <div class="card-header bg-transparent border-bottom">
        <div class="d-flex flex-wrap align-items-start">
            <div class="me-2">
                <h5 class="card-title mt-2 text-primary">STOCK DETAILS</h5>
            </div>
            <div class="ms-auto">
                <form action="{{url('/')}}/get_stock_filter_result_table" method="post">
                    @csrf
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table id="result_table" class="table table-sm">
                <thead>
                    <tr>
                        <th style="white-space: nowrap;">PRODUCT CODE <span class="filter-icon"><i class="bx bx-filter"></i></span></th>
                        ...
                        <th style="white-space: nowrap;">PUBLISHER <span class="filter-icon"><i class="bx bx-filter"></i></span></th>
                        <th style="white-space: nowrap;">MEDIUM <span class="filter-icon"><i class="bx bx-filter"></i></span></th>
                        <th style="white-space: nowrap;">GRADES <span class="filter-icon"><i class="bx bx-filter"></i></span></th>
                        <th style="white-space: nowrap;">SUBJECTS <span class="filter-icon"><i class="bx bx-filter"></i></span></th>
                        ...
                        <th style="white-space: nowrap;">AVAILABIL QUANTITY <span class="filter-icon"><i class="bx bx-filter"></i></span></th>
                    </tr>
                </thead>
                <tbody></tbody>
            </table>
    ...
    function get_table_data() {
        ...
        var table = $('#result_table').DataTable({
            ...
            columnDefs: [{
                ...
                {
                    targets: [8, 9], // ✅ right-align SELLING PRICE & AVAIL QTY
                    className: 'text-end'
                }
            ],
            language: {
            ...
            ajax: {
                ...
                url: "{{url('/')}}/get_stock_filter_result_table",
                data: {
                    FROM_DATE: '{{$FROM_DATE}}',
                    TO_DATE: '{{$TO_DATE}}',
                    MW_ID: '{{$MW_ID}}',
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type) {
                        ...
                        let value = parseFloat(data);
                        if (type === 'display') {
                            return `<span data-value="${value}">${value.toLocaleString('en-US', { minimumFractionDigits: 0 })}</span>`;
                        }
                        return value;
                    }
                }
            ...
            initComplete: function() {
                ...
                $('#result_table thead th').each(function(i) {
                    ...
                    api.column(i).data().unique().sort().each(function(d) {
                        ...
                        // ✅ numeric logic for column 8 & 9
                        if (i === 8 || i === 9) {
                            value = parseFloat(d);
                            displayText = isNaN(value) ?
                                'N/A' :
                                value.toLocaleString('en-US', {
                                    minimumFractionDigits: i === 8 ? 2 : 0
                                });
                        }

                        $('<label><input type="checkbox" value="' + value + '"> ' + displayText + '</label>')
                            .appendTo(dropdown);
                    });

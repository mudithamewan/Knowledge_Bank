...
<div class="card">
    <div class="card-header bg-transparent border-bottom">
        <div class="d-flex flex-wrap align-items-start">
            <div class="me-2">
                <h5 class="card-title mt-2 text-primary">PRODUCT DETAILS</h5>
            </div>
            <div class="ms-auto">
                <form action="{{url('/')}}/get_product_filter_result_table" method="post">
                    @csrf
            ...
            <table id="result_table" class="table table-sm">
                <thead>
                    <tr>
                        ...
                        <th style="white-space: nowrap;">AUTHOR <span class="filter-icon"><i class="bx bx-filter"></i></span></th>
    ...
    function get_table_data() {
        ...
        var table = $('#result_table').DataTable({
            ...
            ajax: {
                ...
                url: "{{url('/')}}/get_product_filter_result_table",
                data: {
                    FROM_DATE: '{{$FROM_DATE}}',
                    TO_DATE: '{{$TO_DATE}}',
                    P_ID: '{{$P_ID}}',
            ...
            columns: [{
                    data: 'p_id',
                    render: function(d) {
                        return '<a href="<?= url('/') ?>/Product_Profile/' +
            ...
            initComplete: function() {
                ...
                $('#result_table thead th').each(function(i) {
                    // ACTION column is index 13 -> skip
                    if (i === 15) return;

                    var $th = $(this);
                    var dropdown = $('<div class="dropdown-filter"></div>');

                    // attach dropdown to body so it won't be clipped by scroll container

...
<script>
    $(document).ready(function() {
        ...
        // 👇 Default customer: NO CUSTOMER with id 0
        let orderCustomers = [{
            customer_id: 0,
        ...
        // Remove customer (disabled for NO CUSTOMER)
        $(document).on('click', '.remove-customer', function(e) {
            e.stopPropagation();
            const idx = $(this).data('idx');

            // Don't remove NO CUSTOMER (id = 0)
            if (orderCustomers[idx].customer_id === 0) {
                return;
            }
        ...
        // 👇 Initial render so NO CUSTOMER shows from start
        renderOrderCustomers();
        renderActiveCustomerCart();
